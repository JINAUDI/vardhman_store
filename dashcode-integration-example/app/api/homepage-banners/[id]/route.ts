...
export async function PUT(
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .update(mapHomepageBannerToSupabasePayload(body))
      .eq("id", id)
      .select("*")
      .single();
    ...
    return NextResponse.json(
      {
        status: "fail",
        message: getReadableSupabaseErrorMessage(error, "Unable to update homepage banner in Supabase."),
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { status: "success", message: "Homepage banner deleted successfully", data: { deleted: true, id }, storage: "supabase" },
      { headers: CORS_HEADERS }
    );
  } catch (error) {
    return NextResponse.json(
      {
        status: "fail",
        message: getReadableSupabaseErrorMessage(error, "Unable to delete homepage banner from Supabase."),
        data: error instanceof Error ? error.message : error,
      },
      { status: 400, headers: CORS_HEADERS }
    );
